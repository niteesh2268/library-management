<?php
	...
	if($_POST["return"]){
		...
		while($row = $result1->fetch_assoc()) {
			if ($row["rollnumber"] === $roll && $row["id"] === $returnid) {
		...
		if($result->num_rows === 0) {
			...
			$still=$claim-$number;
			$sql8="UPDATE claimers SET numberofresourcesclaimed=numberofresourcesclaimed-".$number." WHERE id=".$returnid;
		...
		if($ava >= $number){
			...
			while($row = $result1->fetch_assoc()) {
				if($row["rollnumber"] == $roll && $row["idofresourceclaimed"] == $resourceid){
					$index = $row["id"];
					$resource=$row["resourcetypeclaimed"];
					break;
				}
			}
			if($index !== 0 && $resource==$type){
